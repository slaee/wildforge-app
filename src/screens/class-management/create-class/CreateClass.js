...
function CreateClass({ visible, handleModal }) {
  ...
  const renderShowCode = () => (
    <Dialog className='code-modal p-3' visible={showCode} onHide={closeShowCodeModal} showHeader={false}>
      ...
      <div className='d-flex flex-row my-3 justify-content-center'>
        ...
        <button className='btn btn-create-primary fw-semibold ms-2' onClick={handleCopyCode}>Copy</button>
      </div>
    </Dialog>
  );
      ...
      <Dialog className='create-modal p-5' visible={visible} onHide={handleModal} showHeader={false}>
        <div className='d-flex flex-column'>
          ...
          <InputText
            ...
            onChange={handleInputChange}
          />
          <button className='btn btn-create-primary ms-auto fw-semibold my-3' onClick={openShowCodeModal}>Create</button>
        </div>
      </Dialog>
